...
function isHariLibur($connection, $tanggal, $tanggal_selesai) {
    ...
    for ($i = $start; $i <= $end; $i += 86400) {
        ...
        $queryHari = "SELECT jam_sekolah_id FROM jam_sekolah WHERE hari = '$namaHari' AND tipe='guru' AND active = 'N' LIMIT 1";
...
$query_izin ="SELECT * FROM izin_pegawai WHERE pegawai_id='$data_user[pegawai_id]' $filter ORDER BY izin_id DESC LIMIT 10";
...
$query_count    ="SELECT COUNT(izin_id) AS total FROM izin_pegawai WHERE pegawai_id='$data_user[pegawai_id]' AND izin_id < $id $filter ORDER BY izin_id DESC";
$result_count   = $connection->query($query_count);
$data_count     = $result_count->fetch_assoc();
$totalRowCount  = $data_count['total'];

$showLimit = 10;
$query_izin ="SELECT * FROM izin_pegawai WHERE pegawai_id='$data_user[pegawai_id]' AND izin_id < $id $filter ORDER BY izin_id DESC LIMIT $showLimit";
$result_izin = $connection->query($query_izin);
      ...
      if (isHariLibur($connection, $tanggal, $tanggal_selesai)) {
       ...
       if (!in_array($fileExt, $allowedTypes)) {
        ...
        /** Cek Izin Hari ini berdasarkan tanggal sekarang */
          $cek_izin = "SELECT izin_id FROM izin_pegawai WHERE pegawai_id='{$data_user['pegawai_id']}' AND ('$tanggal' BETWEEN tanggal AND tanggal_selesai)";
            if ($connection->query($cek_izin)->num_rows > 0) {
             ...
             $add ="INSERT INTO izin_pegawai(pegawai_id,
                  ...
                  keterangan,
                  time,
                  date) values('{$data_user['pegawai_id']}',
                  '$data_user[nama_lengkap]',
                  '$tanggal',
                  '$tanggal_selesai',
                  '$newFileName',
...
if(isset($_POST['id'])){
  ...
  $query_izin  = "SELECT * FROM izin_pegawai WHERE pegawai_id='$data_user[pegawai_id]' AND izin_id='$id'";
  $result_izin = $connection->query($query_izin);
  if($result_izin->num_rows > 0){
    while ($data_izin = $result_izin->fetch_assoc()) {
...
if (empty($error)){
    ...
    /** Hapus file */
    if (!empty($_FILES['foto']['name'])) {
      $query_izin ="SELECT files FROM izin_pegawai WHERE pegawai_id='{$data_user['pegawai_id']}' AND izin_id='$id'";
      $result_izin = $connection->query($query_izin);
      if($result_izin->num_rows > 0){
    ...
    $update="UPDATE izin_pegawai SET tanggal='$tanggal',
    ...
    $update .= "WHERE pegawai_id='{$data_user['pegawai_id']}' AND izin_id='$id'"; 
...
case 'delete':
  if(isset($_POST['id'])){
  $id       = anti_injection(convert("decrypt",$_POST['id']));
  $query_izin ="SELECT izin_id, files FROM izin_pegawai WHERE pegawai_id='{$data_user['pegawai_id']}' AND izin_id='$id'";
  $result_izin = $connection->query($query_izin);
  if($result_izin->num_rows > 0){
      $data_izin = $result_izin->fetch_assoc();

      $deleted = "DELETE FROM izin_pegawai WHERE pegawai_id='{$data_user['pegawai_id']}' AND izin_id='$id'";
