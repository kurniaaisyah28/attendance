...
function tambahAbsen($connection, $user_id, $tanggal, $lokasi_id, $keterangan) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        VALUES ('$user_id', '$tanggal', '$lokasi_id', 'Izin', 'Izin', 'Izin', '$keterangan')";
...
$fields = [
  ...
  if (empty($error)) {
    ...
    $data_siswa = NULL;
    $query_user   = "SELECT nama_lengkap,lokasi FROM user WHERE user_id='$user_id'";
    ...
    if($connection->query($add) === false) { 
        ...
        while (strtotime($tanggal) <= strtotime($tanggal_selesai)) {
          if (!tambahAbsen($connection, $user_id, $tanggal, $data_user['lokasi'], $keterangan))
          $status = '';
          break;
...
/** Data Update izin */
  ...
  if($result_izin->num_rows > 0){
    $data_izin = $result_izin->fetch_assoc();
      ...
      $data['user_id']        = htmlentities($data_izin["user_id"]??'-');
...
if (isset($_POST['id'])) {
  ...
  // Tambah notifikasi
    ...
    'Permohonan Izin Anda disetujui', 'izin', '$date', '$timeNow', 'siswa', 'siswa', 'N')";
    $connection->query($notifSql);

  // Tambah absen per hari
  while (strtotime($tanggal) <= strtotime($tglSelesai)) {
      if (!tambahAbsen($connection, $user_id, $tanggal, $dataUser['lokasi'], $keterangan)) break;
...
if (isset($_POST['id'])) {
  ...
  $notifSql = "INSERT INTO notifikasi (user_id, nama, keterangan, link, tanggal, datetime, tipe, tujuan, status)
  VALUES ('{$dataIzin['user_id']}','{$dataUser['nama_lengkap']}',
  'Permohonan Izin Anda ditolak', 'izin', '$date', '$timeNow', 'siswa', 'siswa', 'N')";
  $connection->query($notifSql);
...
if(isset($_POST['id'])){
  ...
  $id = anti_injection(epm_decode($_POST['id']));
  $q = $connection->query("SELECT * FROM izin WHERE izin_id='$id'");
  if ($q->num_rows === 0) {
      exit('Data tidak ditemukan!');
  }

  $d = $q->fetch_assoc();
  if ($d['status'] !== 'N' && $d['status'] !== '-') {
      exit('Data izin telah disetujui dan tidak dapat dihapus!');
  }

  // Hapus data cuti
  if ($connection->query("DELETE FROM izin WHERE izin_id='$id'")) {
      echo 'success';
    ...
    $end->modify('+1 day');
      foreach (new DatePeriod($start, new DateInterval('P1D'), $end) as $date) {
          ...
          $connection->query("DELETE FROM absen WHERE tanggal='$tgl' AND user_id='{$d['user_id']}'");
      }
  } else {
