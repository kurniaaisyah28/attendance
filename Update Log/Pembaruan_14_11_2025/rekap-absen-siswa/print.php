...
LEFT JOIN user ON absen.user_id=user.user_id $filter ORDER BY user.nama_lengkap ASC";
...
if(file_exists('../../../sw-content/'.($site_kop??'-logo.png').'')){
  ...
  $pdf->Cell(0, 7, 'LAPORAN KEHADIRAN KELAS '.$data_user['wali_kelas'].'', 0, 1, 'C');
  $pdf->SetFont('Arial', '', 10);
  $pdf->Cell(0, 6, 'Tanggal '.($_GET['tanggal']??'0'), 0, 1, 'C');
  $pdf->Ln(5);
  ...
  $header = [
      'No', 'Nama Siswa', 'Jam Sekolah', 'Absen Masuk', 'Absen Pulang', 'Kehadiran', 'Lokasi IN', 'Lokasi OUT' ,'Keterangan'];

  $widths = [10, 50, 30, 36, 36, 30, 20, 20, 45];
...
while ($data_absen = $result_absen->fetch_assoc()) {$no++;
  ...
  // Map IN
  if (!empty($data_absen['latitude_longtitude_in']) && $data_absen['latitude_longtitude_in'] !== '-') {
      $map_in_url = 'https://www.google.com/maps/place/' . $data_absen['latitude_longtitude_in'].'';
  } else {
      $map_in_url = '-';
  }

  // Map OUT
  if (!empty($data_absen['latitude_longtitude_out']) && $data_absen['latitude_longtitude_out'] !== '-') {
      $map_out_url = 'https://www.google.com/maps/place/' . $data_absen['latitude_longtitude_out'].'';
  } else {
      $map_out_url = '-';
  }
    ...
    $pdf->Cell($widths[3], 7, ''.strip_tags($data_absen['absen_in']??'-').' ('.strip_tags($status_masuk??'').')', 1, 0, 'C', false);
    $pdf->Cell($widths[4], 7, ''.strip_tags($data_absen['absen_out']??'-').' ('.strip_tags($status_pulang??'').')', 1, 0, 'C', false);
    $pdf->Cell($widths[5], 7, strip_tags($kehadiran??'-'), 1, 0, 'C', false);
    if ($map_in_url !=='-') {
        $pdf->SetTextColor(0, 0, 255); // Biru seperti hyperlink
        $pdf->SetFont('', 'U');        // Underline
        $pdf->Cell($widths[6], 7, 'Lokasi in', 1, 0, 'C', false, $map_in_url);
    } else {
        $pdf->SetTextColor(0);
        $pdf->SetFont('');
        $pdf->Cell($widths[6], 7, '-', 1, 0, 'C', false);
    }
...
// Link OUT
     if ($map_out_url !=='-') {
        ...
        $pdf->SetFont('', 'U');
        $pdf->Cell($widths[7], 7, 'Lokasi Out', 1, 0, 'C', false, $map_out_url);
    } else {
        $pdf->SetTextColor(0);
        $pdf->SetFont('');
    ...
    $pdf->SetFont('');
     $pdf->Cell($widths[8], 7, strip_tags($keterangan??''), 1, 0, 'L', false);
    $pdf->Ln();
    ...
    $pdf->Output('I', 'Laporan_Absen_'.$data_user['wali_kelas'].'_'.($_GET['tanggal']??'-').'.pdf');
