        ...
        <?php } else {
            ...
            async function processLocationAndQRCode() {
                try {
                    ...
                    if (!isLocationValid) {
                        notify('❌ Lokasi Anda tidak terdeteksi dengan benar.', 'error');
                        return;
                    }
            ...
            if ("geolocation" in navigator) {
                navigator.geolocation.getCurrentPosition(async pos => {
                    ...
                    if(valid){
                        ...
                        window.scrollTo(0,0)}
                        $('.btn-cameraFlip').click(function(){if(webcam.webcamList.length>1){webcam.flip();webcam.start()}});$(".btn-take-photo").click(()=>{removeCapture();let picture=webcam.snap(300,300);var img=new Image();img.src=picture;var canvas=document.getElementById("canvas");var ctx=canvas.getContext("2d");ctx.beginPath();ctx.arc(100,100,50,1.5*Math.PI,0.5*Math.PI,!1);const imgData=canvasElement.toDataURL();$.ajax({type:"POST",url:"./module/absen-in/sw-proses.php?action=absen-selfie",data:{img:imgData,username:'-',latitude:latitude,radius:0,road:road,city:city,radius_aktif:'N'},success:function(data){const[status,message]=data.split("/");
                            if(status==='success'){
