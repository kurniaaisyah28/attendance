...
$filterParts[] = "absen_ekbm.kelas='$kelas' AND absen_ekbm.tanggal='$tanggal'";


$filter = 'WHERE ' . implode(' AND ', $filterParts);

$query_absen ="SELECT absen_ekbm.*,user.nama_lengkap,user.nisn,user.kelas FROM absen_ekbm 
LEFT JOIN user ON absen_ekbm.user_id=user.user_id $filter ORDER BY user.nama_lengkap ASC";
...
if(file_exists('../../../sw-content/'.($site_kop??'-logo.png').'')){
  ...
  $pdf->Cell(0, 7, 'LAPORAN KEHADIRAN EKBM KELAS '.$kelas.'', 0, 1, 'C');
  $pdf->SetFont('Arial', '', 10);
  $pdf->Cell(0, 6, ''.format_hari_tanggal($_GET['tanggal']??'-'), 0, 1, 'C');
  $pdf->Ln(5);
  ...
  $header = [
      'No', 'Nama Siswa', 'NISN', 'Pelajaran', 'Kelas', 'Keterangan'];

  $widths = [10, 50, 35, 45, 25, 25];
...
while ($data_absen = $result_absen->fetch_assoc()) {$no++;
    ...
    $pdf->Cell($widths[4], 7, strip_tags($data_absen['kelas']??'-'), 1, 0, 'C', false);
    $pdf->Cell($widths[5], 7, strip_tags($keterangan??'-'), 1, 0, 'C', false);
    ...
    $pdf->Cell(105, 10, strip_tags($data_user['nama_lengkap']??''), 0, 0, 'L');
