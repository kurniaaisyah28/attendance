      ...
      if (!$data_jam) {
        ...
        if ($_POST['radius_aktif']=='Y') {
          ...
          if ($data_pegawai['lokasi_radius'] > $radius) {
            ...
            $query_absensi = "SELECT absen_id,absen_out FROM absen_pegawai WHERE tanggal='$date' AND pegawai_id='".htmlspecialchars($data_user['pegawai_id'], ENT_QUOTES, 'UTF-8')."' LIMIT 1";
              ...
              if ($result_absensi->num_rows > 0) {
                ...
                if (is_null($data_absen['absen_out'])){
                    $update ="UPDATE absen_pegawai SET absen_out='$time_absen',
                        ...
                        map_out='$latitude',
                        radius_out='$radius'
                        WHERE tanggal='$date' AND pegawai_id='".htmlspecialchars($data_user['pegawai_id'], ENT_QUOTES, 'UTF-8')."' 
...
if (empty($_POST['img'])){
    ...
    /* -------- Upload Foto  -------*/
    $foto = 'pulang_' .$data_user['pegawai_id']. '_' . date('Y-m-d') . '_' . uniqid() . '.png';
...
if (empty($error)) {
  ...
  $query_pegawai = "SELECT pegawai.pegawai_id,lokasi.*
    ...
    // *** Absen Tanpa Radius ***
      ...
      if ($result_absensi->num_rows > 0) {
          ...
          if (is_null($data_absen['absen_out'])){
              $update ="UPDATE absen_pegawai SET absen_out='$time_absen',
                  ...
                  pegawai_id='".htmlspecialchars($data_user['pegawai_id'], ENT_QUOTES, 'UTF-8')."' 
                  AND absen_id='$data_absen[absen_id]'";
      ...
      echo 'Data Pegawai tidak ditemukan, silahkan hubungi Admin!';
  }
