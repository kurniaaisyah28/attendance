...
$query_histori ="SELECT * FROM absen_pegawai WHERE pegawai_id='$data_user[pegawai_id]' $filter ORDER BY absen_id DESC LIMIT 15";
...
$query_count    ="SELECT COUNT(absen_id) AS total FROM absen_pegawai WHERE pegawai_id='$data_user[pegawai_id]' $filter AND absen_id < $id ORDER BY absen_id DESC";
$result_count   = $connection->query($query_count);
$data_count     = $result_count->fetch_assoc();
$totalRowCount  = $data_count['total'];

$showLimit = 15;
$query_histori ="SELECT * FROM absen_pegawai WHERE pegawai_id='$data_user[pegawai_id]' $filter AND absen_id < $id ORDER BY absen_id DESC LIMIT $showLimit";
$result_histori = $connection->query($query_histori);
...
if(isset($_POST['id']) && !empty($_POST['id'])){  
  ...
  $query  = "SELECT * FROM absen_pegawai WHERE pegawai_id='".htmlentities($data_user['pegawai_id'], ENT_QUOTES, 'UTF-8')."' AND absen_id='$id'";
...
case 'update':
  $error = array();
    ...
    if (empty($error)) {
      $update="UPDATE absen_pegawai SET keterangan='$keterangan' WHERE absen_id='$id' AND pegawai_id='".htmlentities($data_user['pegawai_id'], ENT_QUOTES, 'UTF-8')."'"; 
