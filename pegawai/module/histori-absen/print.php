...
$query_absen ="SELECT * FROM absen_pegawai WHERE pegawai_id='$data_user[pegawai_id]' $filter";
...
while ($data_absen = $result_absen->fetch_assoc()) {$no++;
  ...
  if (!empty($data_absen['latitude_longtitude_out']) && $data_absen['latitude_longtitude_out'] !== '-') {
    ...
    $pdf->Cell($widths[1], 7, ''.$data_absen['tanggal'], 1, 0, 'C', false);
