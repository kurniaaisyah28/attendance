...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    $aColumns = ['id', 'kode', 'template', 'tipe', 'date'];
    $sIndexColumn = "id";
    $sTable = "template_surat";
    $gaSql['user'] = DB_USER;
    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
      extract($aRow);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            if($modifikasi =='Y'){
                $btn_update = '<a href="javascript:void(0)" data-id="'.htmlentities(convert('encrypt', $aRow['id']??'-')).'" class="table-action table-action-primary btn-update" data-toggle="tooltip" title="Edit">
            ...
            if($hapus =='Y'){
                $btn_hapus ='<a href="javascript:void(0)" class="table-action table-action-delete btn-tooltip btn-delete" data-toggle="tooltip" data-id="'.htmlentities(convert('encrypt', $aRow['id']??'-')).'" title="Hapus">
            ...
            $row[] = strip_tags($aRow['kode']??'-');
            $row[] = ucwords($aRow['tipe']??'-');
