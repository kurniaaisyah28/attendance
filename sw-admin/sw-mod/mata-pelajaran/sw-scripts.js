    ...
    $(document).ready(function() {
        ...
        table = $('.datatable').DataTable({
           ...
           // "order": [[1, 'desc']],
            ...
            "ajax": {
                "url": "./sw-mod/mata-pelajaran/sw-datatable.php",
                "type": "POST",
                "data": {
...
$(document).on('click', '.btn-reset', function(){
    ...
    function submitForm_Add() { 
        ...
        $.ajax({    
            type : 'POST',
            url  : './sw-mod/mata-pelajaran/sw-proses.php?action=add',
            data : data,
            cache: false,
            async: false,
            beforeSend: function() { 
...
$(document).on('click', '.btn-update', function(){
    ...
    $.ajax({
        type: 'POST',
        url  : './sw-mod/mata-pelajaran/sw-proses.php?action=get-data-update',
        data: {id:id},
        dataType:'json',
    ...
    var id = $(this).attr("data-id");
      ...
      .then((value) => {
        if(value) {
            loading();
            $.ajax({  
                 url:'./sw-mod/mata-pelajaran/sw-proses.php?action=delete',
                 type:'POST',    
                 data:{id:id},  
                success:function(data){ 
                    if (data == 'success') {
