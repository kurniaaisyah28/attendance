...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    $aColumns = ['izin_id', 'pegawai_id', 'tanggal','tanggal_selesai','alasan', 'keterangan', 'files','time', 'date','status'];
...
while ($aRow = mysqli_fetch_array($rResult)){$no++;
    ...
    for ($i=1 ; $i<count($aColumns) ; $i++){
        ...
        $query_pegawai = "SELECT pegawai_id,nama_lengkap FROM pegawai WHERE pegawai_id='$aRow[pegawai_id]'";
        ...
        if($aRow['status'] == 'PENDING'){
            $status = '<span class="badge'.htmlspecialchars($aRow['izin_id']).' badge badge-info">Pending</span>';
