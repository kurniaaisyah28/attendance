...
function loadData() {
    ...
    $(document).ready(function () {
        ...
        table = $('.datatable').DataTable({
            ...
            "ajax": {
                "url": "./sw-mod/bentuk-pelanggaran/sw-datatable.php",
                "type": "POST",
                "data": {
...
function submitForm_Add() {
    ...
    $.ajax({
        type: 'POST',
        url: './sw-mod/bentuk-pelanggaran/sw-proses.php?action=add',
        data: data,
        cache: false,
        async: false,
        beforeSend: function () {
...
$(document).on('click', '.btn-update', function () {
    ...
    $('.modal-title').html('Update Bentuk Pelanggaran');
    $.ajax({
        type: 'POST',
        url: './sw-mod/bentuk-pelanggaran/sw-proses.php?action=get-data-update',
        data: { id: id },
        dataType: 'json',
        success: function (response) {
            $('.id').val(response.bentuk_pelanggaran_id);
            $('.kategori').val(response.kategori_pelanggaran_id);
    ...
    swal({
        ...
        .then((value) => {
            if (value) {
                loading();
                $.ajax({
                    url: './sw-mod/bentuk-pelanggaran/sw-proses.php?action=delete',
                    type: 'POST',
                    data: { id: id },
                    success: function (data) {
                        if (data == 'success') {
