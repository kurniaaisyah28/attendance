...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    $aColumns = ['kategori_pelanggaran_id', 'nama_kategori'];
    $sIndexColumn = "kategori_pelanggaran_id";
    $sTable = "kategori_pelanggaran";
    $gaSql['user'] = DB_USER;
    ...
    $sOrder = "ORDER BY kategori_pelanggaran_id DESC";
    if (isset($_GET['iSortCol_0']))
    {
        ...
        if ($sOrder == "ORDER BY kategori_pelanggaran_id DESC")
        {
            $sOrder = "ORDER BY kategori_pelanggaran_id DESC";
        }
    }
    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
      extract($aRow);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            if($modifikasi =='Y'){
                $btn_update = '<a href="javascript:void(0)" data-id="'.htmlentities(convert('encrypt', $aRow['kategori_pelanggaran_id']??'-')).'" class="table-action table-action-primary btn-update" data-toggle="tooltip" title="Edit">
            ...
            if($hapus =='Y'){
                $btn_hapus ='<a href="javascript:void(0)" class="table-action table-action-delete btn-tooltip btn-delete" data-toggle="tooltip" data-id="'.htmlentities(convert('encrypt', $aRow['kategori_pelanggaran_id']??'-')).'" title="Hapus">
            ...
            $row[] = strip_tags($aRow['nama_kategori']??'-');
