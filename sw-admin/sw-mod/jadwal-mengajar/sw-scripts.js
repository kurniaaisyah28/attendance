    ...
    $(document).ready(function() {
        ...
        table = $('.datatable').DataTable({
           ...
           // "order": [[1, 'desc']],
            ...
            "ajax": {
                "url": "./sw-mod/jadwal-mengajar/sw-datatable.php",
                "type": "POST",
                "data": {
...
$(".hari, .mata_pelajaran, .pegawai, .kelas").change(function(){
 loadData();
});
...
$(document).on('click', '.btn-reset', function(){
    ...
    function submitForm_Add() { 
        ...
        $.ajax({    
            type : 'POST',
            url  : './sw-mod/jadwal-mengajar/sw-proses.php?action=add',
            data : data,
            cache: false,
            async: false,
            beforeSend: function() { 
...
$(document).on('click', '.btn-update', function(){
    ...
    $.ajax({
        type: 'POST',
        url  : './sw-mod/jadwal-mengajar/sw-proses.php?action=get-data-update',
        data: {id:id},
        dataType:'json',
        success: function(response) {
            $('.id').val(response.jadwal_id);
            $('.hari').val(response.hari);
            $('.mata_pelajaran').val(response.mata_pelajaran);
            $('.pegawai').val(response.pegawai);
            $('.kelas').val(response.kelas);
            $('.dari_jam').val(response.dari_jam);
            $('.sampai_jam').val(response.sampai_jam);
    ...
    var id = $(this).attr("data-id");
      ...
      .then((value) => {
        if(value) {
            loading();
            $.ajax({  
                 url:'./sw-mod/jadwal-mengajar/sw-proses.php?action=delete',
                 type:'POST',    
                 data:{id:id},  
                success:function(data){ 
                    if (data == 'success') {
