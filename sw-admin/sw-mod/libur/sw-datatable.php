...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    $aColumns = ['libur_nasional_id', 'libur_tanggal','keterangan'];
    $sIndexColumn = "libur_nasional_id";
    $sTable = "libur_nasional";
    $gaSql['user'] = DB_USER;
    ...
    $sOrder = "ORDER BY libur_nasional_id DESC";
    if (isset($_GET['iSortCol_0']))
    {
        ...
        if ($sOrder == "ORDER BY libur_nasional_id DESC")
        {
            $sOrder = "ORDER BY libur_nasional_id DESC";
        }
    }
    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
      extract($aRow);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            if($modifikasi =='Y'){
                $btn_update = '<a href="javascript:void(0)" data-id="'.htmlentities(convert('encrypt', $aRow['libur_nasional_id']??'-')).'" class="table-action table-action-primary btn-update" data-toggle="tooltip" title="Edit">
            ...
            if($hapus =='Y'){
                $btn_hapus ='<a href="javascript:void(0)" class="table-action table-action-delete btn-tooltip btn-delete" data-toggle="tooltip" data-id="'.htmlentities(convert('encrypt', $aRow['libur_nasional_id']??'-')).'" title="Hapus">
            ...
            $row[] = ''.format_hari_tanggal($aRow['libur_tanggal']??'-').'';
