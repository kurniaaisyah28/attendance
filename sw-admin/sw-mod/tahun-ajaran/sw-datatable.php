...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    $aColumns = ['tahun_ajaran_id', 'tahun_ajaran'];
    $sIndexColumn = "tahun_ajaran_id";
    $sTable = "tahun_ajaran";
    $gaSql['user'] = DB_USER;
    ...
    $sOrder = "ORDER BY tahun_ajaran_id DESC";
    if (isset($_GET['iSortCol_0']))
    {
        ...
        if ($sOrder == "ORDER BY tahun_ajaran_id DESC")
        {
            $sOrder = "ORDER BY tahun_ajaran_id DESC";
        }
    }
      ...
      extract($aRow);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            if($modifikasi =='Y'){
                ...
                $btn_update = '<a href="javascript:void(0)" data-id="'.htmlentities(convert('encrypt', $aRow['tahun_ajaran_id']??'-')).'" class="table-action table-action-primary btn-update" data-toggle="tooltip" title="Edit">
            ...
            if($hapus =='Y'){
                $btn_hapus ='<a href="javascript:void(0)" class="table-action table-action-delete btn-tooltip btn-delete" data-toggle="tooltip" data-id="'.htmlentities(convert('encrypt', $aRow['tahun_ajaran_id']??'-')).'" title="Hapus">
