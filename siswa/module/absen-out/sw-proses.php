...
if($row_site['tipe_absen_siswa']=='qrcode'){
  $filter ="AND user.nisn = '$qrcode'";
}else{
  $filter ="AND lokasi.lokasi_qrcode = '$qrcode'";
}

if (empty($error)) {
    $query_siswa = "SELECT user.user_id,lokasi.*
    FROM user LEFT JOIN lokasi ON user.lokasi = lokasi.lokasi_id 
    WHERE user.user_id='".htmlspecialchars($data_user['user_id'], ENT_QUOTES, 'UTF-8')."' $filter";
      ...
      // menentukan status telat s-widodo.com
       $status_out = ($time_sekarang < $data_jam['jam_pulang']) ? 'Pulang Cepat' : 'Tepat Waktu';
        ...
        if ($_POST['radius_aktif']=='Y') {
            ...
            if ($data_siswa['lokasi_radius'] > $radius) {
              // Cek absensi hari ini
              $query_absensi = "SELECT absen_id,absen_out FROM absen WHERE tanggal='$date' AND user_id='".htmlspecialchars($data_user['user_id'], ENT_QUOTES, 'UTF-8')."' LIMIT 1";
                ...
                if ($result_absensi->num_rows > 0){
                    ...
                    if (is_null($data_absen['absen_out'])){
                      ...
                      $update ="UPDATE absen SET absen_out='$time_absen',
                          ...
                          map_out='$latitude',
                          radius_out='$radius'
                          WHERE tanggal='$date' AND user_id='".htmlspecialchars($data_user['user_id'], ENT_QUOTES, 'UTF-8')."' AND absen_id='$data_absen[absen_id]'";
                    ...
                    echo "success/Terima kasih, {$data_user['nama_lengkap']},\nAbsensi Pulang telah berhasil tercatat pada tanggal " . tanggal_ind($date) . " pukul {$data_absen['absen_in']}!";
                  }
              } else {
...
if (empty($_POST['img'])){
    ...
    /* -------- Upload Foto  -------*/
    $foto = 'pulang_' .$data_user['user_id'].'_' .date('Y-m-d'). '.png';
...
if (empty($error)) {
  ...
  $query_siswa = "SELECT user.user_id,lokasi.*
    FROM user LEFT JOIN lokasi ON user.lokasi = lokasi.lokasi_id 
    WHERE user.user_id='".htmlspecialchars($data_user['user_id'], ENT_QUOTES, 'UTF-8')."'";
    $result_siswa = $connection->query($query_siswa);
    if ($result_siswa->num_rows > 0) {
        $data_siswa = $result_siswa->fetch_assoc();
       ...
       $status_out = ($time_sekarang < $data_jam['jam_pulang']) ? 'Pulang Cepat' : 'Tepat Waktu';
        ...
        // *** Absen Tanpa Radius ***
          ...
          if ($result_absensi->num_rows > 0) {
            ...
            // Absen sudah ada
            echo "success/Terima kasih, {$data_user['nama_lengkap']}, \nAbsensi Pulang telah berhasil tercatat pada tanggal " . tanggal_ind($date) . " dan Jam : {$data_absen['absen_out']}!";
          }
