...
if(htmlentities($_GET['id'] =='1')){
  $query_frendlist = "SELECT chat_list.*, pegawai.nama_lengkap, pegawai.avatar,pegawai.status FROM chat_list
  INNER JOIN pegawai ON chat_list.pegawai_id = pegawai.pegawai_id
  WHERE chat_list.user_id = '{$data_user['user_id']}'
  ...
  if($result_frendlist->num_rows > 0){
    while($data_frendlist = $result_frendlist->fetch_assoc()){
      ...
      $query_pesan ="SELECT pesan,datetime FROM chat WHERE status_pegawai='N' AND pegawai_id='$data_frendlist[pegawai_id]' AND user_id='$data_user[user_id]' ORDER BY chat_id DESC LIMIT 1";
      ...
      echo'
      <li class="get-open-cat" data-id="'.epm_encode($data_frendlist['pegawai_id']).'">';
...
case 'search-frendlist':
  ...
  $query_frendlist = "SELECT pegawai_id, nama_lengkap, avatar, tanggal_login, status 
                    FROM pegawai 
  ...
  if($result_frendlist->num_rows > 0){
    ...
    echo'
    <li class="add-chat" data-id="'.epm_encode($data_frendlist['pegawai_id']).'">';
        if(file_exists('../../sw-content/avatar/'.$data_frendlist['avatar'].'')){
            echo'<img src="../sw-content/avatar/'.$data_frendlist['avatar'].'" height="40">';
        }else{
            echo'<img src="../sw-content/avatar/avatar.jpg" height="40">';
...
if (isset($_POST['id'])) {
    ...
    $query_frendlist = "SELECT user_id FROM chat_list WHERE pegawai_id='$id'";
    $result_frendlist = $connection->query($query_frendlist);
    if($result_frendlist->num_rows > 0){
      /** Update jika sudah ada di daftar */
      $update="UPDATE chat_list SET datetime='$date $time_sekarang' WHERE pegawai_id='$id' AND user_id='$data_user[user_id]'"; 
...
$query_frendlist = "SELECT chat_list.*,pegawai.nama_lengkap,pegawai.avatar,pegawai.tanggal_login FROM chat_list
INNER JOIN pegawai ON chat_list.pegawai_id = pegawai.pegawai_id WHERE chat_list.pegawai_id='$id' AND chat_list.user_id='$data_user[user_id]'";
$result_frendlist = $connection->query($query_frendlist);
  if($result_frendlist->num_rows > 0){
    $data_frendlist = $result_frendlist->fetch_assoc();
    
    $data['pegawai_id']     = anti_injection(epm_encode($data_frendlist["pegawai_id"]??'-'));
    ...
    if(file_exists('../../sw-content/avatar/'.($data_frendlist["avatar"]??'-.jpg').'')){
      $data['avatar']       = '../sw-content/avatar/'.($data_frendlist["avatar"]).'';
    }else{
...
if (isset($_POST['id'])) {
  $pegawai = anti_injection(epm_decode($_POST['id']));
  $query_pesan ="SELECT * FROM chat WHERE pegawai_id='$pegawai' AND user_id='$data_user[user_id]' ORDER BY chat_id ASC";
  $result_pesan = $connection->query($query_pesan);
  if($result_pesan->num_rows > 0) {
    while($data_pesan = $result_pesan->fetch_assoc()){

      $query_frendlist = "SELECT nama_lengkap,avatar FROM pegawai WHERE pegawai_id='$data_pesan[pegawai_id]'";
      $result_frendlist = $connection->query($query_frendlist);
      $data_frendlist = $result_frendlist->fetch_assoc();

        if($data_pesan['tujuan'] =='pegawai'){
        ...
        echo'
          ...
          $update="UPDATE chat SET status_pegawai='Y' WHERE user_id='$data_user[user_id]' AND chat_id='$data_pesan[chat_id]'"; 
...
if (empty($_POST['pegawai'])) {
    ...
    $pegawai = anti_injection(epm_decode($_POST['pegawai']));
}
