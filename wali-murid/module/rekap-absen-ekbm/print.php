...
INNER JOIN user ON absen_ekbm.user_id=user.user_id AND user.nisn='$data_user[nisn]' $filter ORDER BY user.nama_lengkap ASC";
...
if(file_exists('../../../sw-content/'.($site_kop??'-logo.png').'')){
  ...
  $pdf->Cell(0, 7, 'LAPORAN KEHADIRAN EKBM '.strtoupper($data_siswa['nama_lengkap']??'-').'', 0, 1, 'C');
...
while ($data_absen = $result_absen->fetch_assoc()) {$no++;
    ...
    $query_pelajaran="SELECT nama_mapel FROM mata_pelajaran WHERE id='$data_absen[pelajaran]' limit 1";
    ...
    $pdf->Output('I', 'Laporan_Absen_'.$data_siswa['nama_lengkap'].'_'.($_GET['mulai']??'-').'_'.($_GET['selesai']??'-').'.pdf');
